...
public interface LoggerMapper {
     ...
     */
    @Insert({
        "insert into logger (lid, uid, ",
        "type, time, ip, ",
        "level, result, remark)",
        "values (#{lid,jdbcType=INTEGER}, #{uid,jdbcType=INTEGER}, ",
        "#{type,jdbcType=VARCHAR}, #{time,jdbcType=TIMESTAMP}, #{ip,jdbcType=VARCHAR}, ",
        "#{level,jdbcType=INTEGER}, #{result,jdbcType=INTEGER}, #{remark,jdbcType=VARCHAR})"
    })
    ...
    @Select({
        "select",
        "lid, uid, type, time, ip, level, result, remark",
        "from logger",
        "where lid = #{lid,jdbcType=INTEGER}"
    })
    ...
    List<Logger> selectin(@Param("list1")List<Integer> list1);
}